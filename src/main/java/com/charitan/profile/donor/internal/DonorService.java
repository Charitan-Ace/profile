    ...
    if (redisZSetSize != donorInDB) {
      ...
      if (keyword != null && !keyword.isEmpty()) {
        ...
        if (order.equalsIgnoreCase("ascending")) {
          donorPage =
              filter.equalsIgnoreCase("firstName")
                  ? donorRepository.findByFirstNameContainingIgnoreCase(
                      keyword, PageRequest.of(pageNo, pageSize, Sort.by("firstName").ascending()))
                  : donorRepository.findByLastNameContainingIgnoreCase(
                      keyword, PageRequest.of(pageNo, pageSize, Sort.by("lastName").ascending()));
        } else {
          donorPage =
              filter.equalsIgnoreCase("firstName")
                  ? donorRepository.findByFirstNameContainingIgnoreCase(
                      keyword, PageRequest.of(pageNo, pageSize, Sort.by("firstName").descending()))
                  : donorRepository.findByLastNameContainingIgnoreCase(
                      keyword, PageRequest.of(pageNo, pageSize, Sort.by("lastName").descending()));
        }
      } else {
          ...
          donorPage =
              order.equalsIgnoreCase("ascending")
                  ? donorRepository.findAll(
                      PageRequest.of(pageNo, pageSize, Sort.by("firstName").ascending()))
                  : donorRepository.findAll(
                      PageRequest.of(pageNo, pageSize, Sort.by("firstName").descending()));
        } else {
          donorPage =
              order.equalsIgnoreCase("ascending")
                  ? donorRepository.findAll(
                  ...
                  : donorRepository.findAll(
                      PageRequest.of(pageNo, pageSize, Sort.by("lastName").descending()));
        }
      }

      List<DonorDTO> donors =
          donorPage.getContent().stream().map(DonorDTO::new).collect(Collectors.toList());
      ...
      donorPage
          ...
          .forEach(
              donor -> {
                redisTemplate
                    .opsForValue()
                    .set(DONOR_CACHE_PREFIX + donor.getUserId(), new DonorDTO(donor));
                addToRedisZSet(donor);
              });
