...
public class KafkaConsumer extends AbstractConsumerSeekAware {
  ...
  public void handleDonorCreatedEvent(AuthCreationDto authCreationDto) {
    try {
      ...
      if (roleId.equals("DONOR")) {
        DonorCreationRequest request =
            new DonorCreationRequest(
                id,
                email,
        ...
        CharityCreationRequest request =
            new CharityCreationRequest(
                id,
                email,
  ...
  public void handlePublicKeyChange(String message) {
    try {
      Key jwk = Jwks.parser().build().parse(message).toKey();
  ...
  @SendTo
  public GetCharityProfileByIdsResponseDto getCharityProfileByIds(
      GetCharityProfileByIdsRequestDto request) {
      ...
      if (externalCharityDTO != null) {
        try {
          ...
          OrganizationType organizationType =
              OrganizationType.valueOf(externalCharityDTO.getOrganizationType().name());
          String stripeId = externalCharityDTO.getStripeId();
          String assetKey = externalCharityDTO.getAssetsKey();
          resultList
              .charityProfilesList()
              .add(
                  new CharityProfileDto(
                      id, companyName, address, taxCode, organizationType, stripeId, assetKey));
        } catch (Exception e) {
          logger.error(
              "Failed to get charity #{} profile, error: {}",
              externalCharityDTO.getUserId(),
              e.getMessage());
  ...
  @SendTo
  public GetDonorProfileByIdsResponseDto getDonorProfileByIds(
      GetDonorProfileByIdsRequestDto request) {
    ...
    for (UUID donorId : request.donorIdList()) {
      ...
      if (externalDonorDTO != null) {
        try {
          ...
          resultList
              .donorProfilesList()
              .add(new DonorProfileDto(id, firstName, lastName, address, stripeId, assetKey));
        } catch (Exception e) {
          logger.error(
              "Failed to get donor #{} profile, error: {}",
              externalDonorDTO.getUserId(),
              e.getMessage());
          logger.error("Stacktrace", e);
        }
      }
