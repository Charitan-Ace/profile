...
public class CharityService implements CharityExternalAPI, CharityInternalAPI {
  ...
  private static final String CHARITY_LIST_CACHE_KEY_COMPANY_NAME =
      CHARITY_LIST_CACHE_KEY + ":companyname";
  private static final String CHARITY_LIST_CACHE_KEY_TAX_CODE = CHARITY_LIST_CACHE_KEY + ":taxcode";
  private static final String CHARITY_LIST_CACHE_KEY_ORGANIZATION_TYPE =
      CHARITY_LIST_CACHE_KEY + ":organizationtype";
  ...
  public void createCharity(CharityCreationRequest request) {
    ...
    redisTemplate
        .opsForValue()
        .set(CHARITY_CACHE_PREFIX + request.getUserId(), new CharityDTO(charity));
  ...
  public CharityDTO updateCharity(CharityUpdateRequest request) {
    ...
    Charity charity =
        charityRepository
            ...
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Charity not found."));

    if (!Objects.equals(request.getCompanyName(), charity.getCompanyName())
        && request.getCompanyName() != null) {
      String compositeKey =
          charity.getCompanyName().trim().toLowerCase() + ":" + charity.getUserId();
      ...
      compositeKey =
          charity.getOrganizationType().name().toLowerCase()
              + ":"
    ...
    if (!Objects.equals(request.getTaxCode(), charity.getTaxCode())
    ...
    if (!Objects.equals(
            request.getOrganizationType().trim().toLowerCase(),
            charity.getOrganizationType().name().trim().toLowerCase())
      ...
      String compositeKey =
          charity.getOrganizationType().name().toLowerCase()
              + ":"
              + charity.getCompanyName().trim().toLowerCase()
              + ":"
              + charity.getUserId();
      redisZSetTemplate.opsForZSet().remove(CHARITY_LIST_CACHE_KEY_TAX_CODE, compositeKey);

      compositeKey =
          request.getOrganizationType().toLowerCase()
              + ":"
              + charity.getCompanyName().trim().toLowerCase()
              + ":"
              + charity.getUserId();
    ...
    redisTemplate
        .opsForValue()
        .set(CHARITY_CACHE_PREFIX + charity.getUserId(), new CharityDTO(charity));
  ...
  public Page<CharityDTO> getAll(
    ...
    if (redisZSetSize != charityInDB) {
      ...
      if (keyword != null && !keyword.isEmpty()) {
        ...
        charityPage =
            filter.equalsIgnoreCase("companyName")
                ? charityRepository.findByCompanyNameContainingIgnoreCase(
                    keyword, PageRequest.of(pageNo, pageSize, Sort.by("companyName").ascending()))
                : charityRepository.findByTaxCodeContainingIgnoreCase(
                    keyword, PageRequest.of(pageNo, pageSize, Sort.by("companyName").ascending()));
      } else {
        if (filter.equalsIgnoreCase("companyName")) {
          charityPage =
              order.equalsIgnoreCase("ascending")
                  ...
                  : charityRepository.findByCompanyNameContainingIgnoreCase(
                      keyword,
                      PageRequest.of(pageNo, pageSize, Sort.by("companyName").descending()));
          ...
          charityPage =
              charityRepository.findByOrganizationType(
                  OrganizationType.valueOf(filter.toUpperCase()),
                  PageRequest.of(pageNo, pageSize, Sort.by("companyName").ascending()));
        }
      }

      List<CharityDTO> charities = charityPage.getContent().stream().map(CharityDTO::new).toList();
      ...
      return new PageImpl<>(
          charities, PageRequest.of(pageNo, pageSize), charityPage.getTotalElements());
    }
    ...
    do {
      Cursor<Tuple> scanCursor =
          connection.zScan(cacheKey.getBytes(StandardCharsets.UTF_8), options);
      ...
      while (scanCursor.hasNext()) {
        ...
        String element =
            new String(
                tuple.getValue(),
                StandardCharsets.UTF_8); // Get the value (the member of the sorted set)
      ...
      scanCursor.close();
    } while (!new String(cursor, StandardCharsets.UTF_8)
        .equals("0")); // Continue until the cursor returns "0"
    ...
    if (start >= matchingKeys.size()) {
      ...
      return new PageImpl<>(
          Collections.emptyList(), Pageable.ofSize(end - start + 1), matchingKeys.size());
    }
